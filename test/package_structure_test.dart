...
void main() {
  ...
  group('Package Structure', () {
    test('braven_data directory exists at workspace root', () {
      ...
      expect(dir.existsSync(), isTrue, reason: 'braven_data/ directory must exist');
    });
    ...
    test('pubspec.yaml exists and is valid', () {
      ...
      expect(content.contains('name: braven_data'), isTrue, reason: 'Package name must be braven_data');
    });

    test('Dart SDK constraint matches project (>=3.0.0 <5.0.0)', () {
      final pubspec = File(path.join(packageRoot, 'pubspec.yaml'));
      final content = pubspec.readAsStringSync();
      expect(content.contains('sdk: ">=3.0.0 <5.0.0"'), isTrue, reason: 'SDK constraint must be >=3.0.0 <5.0.0');
    });

    test('test dependency is included', () {
      final pubspec = File(path.join(packageRoot, 'pubspec.yaml'));
      final content = pubspec.readAsStringSync();
      expect(content.contains('test:'), isTrue, reason: 'test package must be in dev_dependencies');
    });

    test('analysis_options.yaml exists with strict typing', () {
      final analysisOptions = File(path.join(packageRoot, 'analysis_options.yaml'));
      ...
      expect(content.contains('strict-casts: true'), isTrue, reason: 'Must enable strict-casts');
      expect(content.contains('strict-inference: true'), isTrue, reason: 'Must enable strict-inference');
      expect(content.contains('strict-raw-types: true'), isTrue, reason: 'Must enable strict-raw-types');
    });
  });

  group('Directory Structure', () {
    test('lib/braven_data.dart entry point exists', () {
      final entryPoint = File(path.join(packageRoot, 'lib', 'braven_data.dart'));
      expect(entryPoint.existsSync(), isTrue, reason: 'Library entry point must exist');

      final content = entryPoint.readAsStringSync();
      expect(content.contains('library'), isTrue, reason: 'Entry point must declare library');
    });

    test('lib/src/ directory exists', () {
      final srcDir = Directory(path.join(packageRoot, 'lib', 'src'));
      expect(srcDir.existsSync(), isTrue, reason: 'Source directory must exist');
    });

    test('test/unit/ directory exists', () {
      final unitDir = Directory(path.join(packageRoot, 'test', 'unit'));
      expect(unitDir.existsSync(), isTrue, reason: 'Unit test directory must exist');
    });

    test('test/benchmarks/ directory exists', () {
      final benchDir = Directory(path.join(packageRoot, 'test', 'benchmarks'));
      expect(benchDir.existsSync(), isTrue, reason: 'Benchmarks directory must exist');
    });
  });
  ...
  group('Package Functionality', () {
    test('dart pub get succeeds (verified by test execution)', () {
      ...
      expect(true, isTrue, reason: 'dart pub get must succeed for tests to run');
    });
