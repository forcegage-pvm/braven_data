...
void main() async {
  ...
  final schema = DelimitedSchema(xColumn: 'timestamp', xType: DataType.timestamp, columns: [
  ...
  // 2. Load "Raw Table"
  // In a real app this would use DelimitedLoader.load(), here we mock the structure
  // derived from "tp-2023646...records.csv"
  final rawTable = await MockDelimitedLoader.load('tp-2023646...csv', schema);
...
class DelimitedSchema {
  ...
  DelimitedSchema({required this.xColumn, required this.xType, required this.columns});
}
...
class MockDelimitedLoader {
  static Future<Map<String, dynamic>> load(String path, DelimitedSchema schema) async {
    return {
